    ...
    <!-- Search and Filter Form -->
     <div class="bg-white p-4 mb-4 rounded shadow">
        <form method="GET" class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            ...
            <!-- Filter Dropdown -->
            <div class="sm:w-48">
                <label class="block text-sm text-gray-700 mb-1">Sort by User Role</label>
                <select
                    name="filter"
        ...
        <table class="min-w-full bg-white border border-gray-200">
            <thead>
                <tr class="bg-red-700 text-white">
                    <th class="py-2 px-4 text-left text-sm font-medium border-r border-red-600">Username</th>
                    <th class="py-2 px-4 text-left text-sm font-medium border-r border-red-600">Password</th>
                    <th class="py-2 px-4 text-center text-sm font-medium border-r border-red-600">User Role</th>
                        ...
                        <tr class="hover:bg-gray-100 transition-all duration-200 hover:scale-[1.02]">
                            ...
                            <td class="py-2 px-4 border-gray-200">
                                <div class="flex items-center justify-center space-x-2">
                                    <!-- Edit button -->
                                    <button command="show-modal" commandfor="edit-dialog-<?= $user['id'] ?>" class="p-1.5 text-blue-600 hover:bg-blue-600 hover:text-white rounded transition-all">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4">
                                    ...
                                    <!-- Delete button -->
                                    <button command="show-modal" commandfor="delete-dialog-<?= $user['id'] ?>" class="p-1.5 text-red-600 hover:bg-red-600 hover:text-white rounded transition-all">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4">
...
<?php foreach ($users as $user): ?>
    <el-dialog>
        <dialog id="edit-dialog-<?= $user['id'] ?>" aria-labelledby="edit-dialog-title-<?= $user['id'] ?>" class="fixed inset-0 size-auto max-h-none max-w-none overflow-y-auto bg-transparent backdrop:bg-transparent">
            <el-dialog-backdrop class="fixed inset-0 bg-gray-900/50 transition-opacity data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in"></el-dialog-backdrop>
                ...
                <el-dialog-panel class="relative w-full max-w-lg transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all">
                    ...
                    <div class="px-6 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="flex items-start space-x-3">
                            ...
                            <div class="text-left">
                                <h3 id="edit-dialog-title-<?= $user['id'] ?>" class="text-lg font-semibold text-gray-900">Edit Account</h3>
                            </div>
                        </div>
                    </div>
                    ...
                    <form action="../controllers/user.controller.php" method="POST" class="px-6 pb-4 space-y-4">
                        ...
                        <div>
                            <label class="block text-sm text-gray-700 mb-1">Username</label>
                            <input type="text" name="username" value="<?= htmlspecialchars($user['username']) ?>" placeholder="Enter a Username"
                        ...
                        <div class="flex flex-col sm:flex-row sm:flex-row-reverse sm:space-x-3 sm:space-x-reverse mt-4">
                            ...
                            <button type="button" command="close" commandfor="edit-dialog-<?= $user['id'] ?>"
                                class="mt-3 sm:mt-0 inline-flex justify-center rounded-md bg-gray-100 px-4 py-2 text-sm font-semibold text-gray-900 hover:bg-gray-200">
                                Cancel
                            </button>
                        </div>
...
<?php foreach ($users as $user): ?>
    <el-dialog>
        <dialog id="delete-dialog-<?= $user['id'] ?>" aria-labelledby="delete-dialog-title-<?= $user['id'] ?>" class="fixed inset-0 size-auto max-h-none max-w-none overflow-y-auto bg-transparent backdrop:bg-transparent">
            <el-dialog-backdrop class="fixed inset-0 bg-gray-900/50 transition-opacity data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in"></el-dialog-backdrop>
                ...
                <el-dialog-panel class="relative w-full max-w-lg transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all">
                    ...
                    <div class="px-6 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="flex items-start space-x-3">
                            ...
                            <div class="text-left">
                                <h3 id="delete-dialog-title-<?= $user['id'] ?>" class="text-lg font-semibold text-gray-900">Delete User</h3>
                                <p class="mt-2 text-sm text-gray-600">
                                    Are you sure you want to delete <span class="font-semibold"><?= htmlspecialchars($user['username']) ?></span>? This action cannot be undone.
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <form action="../controllers/user.controller.php" method="POST" class="px-6 pb-4">
                        ...
                        <div class="flex flex-col sm:flex-row sm:flex-row-reverse sm:space-x-3 sm:space-x-reverse mt-4">
                            ...
                            <button type="button" command="close" commandfor="delete-dialog-<?= $user['id'] ?>"
                                class="mt-3 sm:mt-0 inline-flex justify-center rounded-md bg-gray-100 px-4 py-2 text-sm font-semibold text-gray-900 hover:bg-gray-200">
                                Cancel
                            </button>
                        </div>
